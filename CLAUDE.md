...
assets/favicon.svg     — red spider lily favicon (higanbana, Demon Slayer / Tokyo Ghoul style:
                         crimson petal fan #be123c-#fb7185, pale stamens, dark rounded tile);
                         favicon.ico is the multi-size raster version generated by
                         scripts/gen_favicon.py (PIL, redraws the same 64-unit shapes; rerun it
                         and bump the ?v= on the favicon links in default.html whenever the SVG
                         changes). The lily is the brand mark: it also appears in the hero
                         background and assets/og.png
...
default.html carries the full head: canonical, Open Graph + Twitter cards pointing at
assets/og.png (1200x630, regenerated by scripts/gen_og.py, keep in sync with the favicon
butterfly and rerun when branding changes), and JSON-LD (Person + WebSite on the homepage,
BlogPosting on posts). robots.txt points at the jekyll-sitemap sitemap. Post URLs use
permalink /blog/:title/. PRIVACY applies to structured data too: name is only ever
"Othmane B." / "othmaneb".
